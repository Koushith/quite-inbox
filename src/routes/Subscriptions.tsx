...
import { ExternalLink, Calendar, Mail as MailIcon, Trash2 } from 'lucide-react';
...
export default function SubscriptionsPage() {
  ...
  const [scanTimeWindow, setScanTimeWindow] = useState<'3d' | '7d' | '3m' | '6m' | '12m' | 'all' | undefined>(
    undefined
  );
  ...
  const timeSpanDays =
    oldestDate && newestDate ? Math.ceil((newestDate.getTime() - oldestDate.getTime()) / (1000 * 60 * 60 * 24)) : 0;
        ...
        <div className="bg-gradient-to-r from-blue-50 to-indigo-50 border-b border-blue-100 px-6 py-4">
          <div className="max-w-7xl mx-auto flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <span className="text-sm font-semibold text-blue-900">Scanning your inbox</span>
                <span className="text-xs text-blue-700 ml-2">
                  {scanState.progress.processed} / {scanState.progress.total} messages
                </span>
              </div>
            </div>
            <span className="text-xs text-blue-600 font-medium">Results appear live</span>
          </div>
        </div>
      )}
                ...
                <div className="mb-6 text-center">
                  <div className="inline-flex items-center gap-2 px-4 py-2 bg-blue-50 border border-blue-200 rounded-lg text-sm">
                    ...
                    <span className="font-semibold text-blue-900">
                      Showing emails from{' '}
                      {stats.dateRange.oldest.toLocaleDateString('en-US', {
                        month: 'short',
                        day: 'numeric',
                        year: 'numeric',
                      })}{' '}
                      to{' '}
                      {stats.dateRange.newest.toLocaleDateString('en-US', {
                        month: 'short',
                        day: 'numeric',
                        year: 'numeric',
                      })}
                    </span>
                    <span className="text-blue-600">({stats.dateRange.span} days)</span>
                  </div>
                </div>
              )}
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex items-start gap-3">
                                ...
                                <div className="flex-1 min-w-0">
                                  ...
                                  <div className="text-sm text-gray-500 truncate mt-0.5">{group.domain}</div>
                                </div>
                                {/* Status Badge */}
                                <div
                                  className={`px-3 py-1.5 rounded-lg text-xs font-semibold ${status.bgColor} ${status.color} flex items-center gap-1.5 whitespace-nowrap shadow-sm`}
                                >
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              <strong className="text-destructive">{deleteDialog.group?.messageCount.toLocaleString()}</strong> emails
              from <strong>{deleteDialog.group?.displayName || deleteDialog.group?.domain}</strong>?
              <br />
              <br />
      ...
      <Dialog open={unsubscribeAllDialog} onOpenChange={setUnsubscribeAllDialog}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Are you sure you want to unsubscribe from all{' '}
              {groups.filter((g) => g.unsubscribe.kind !== 'unknown' && !unsubscribedIds.has(g.id)).length}{' '}
              subscriptions?
              <br />
              <br />
          ...
          <DialogHeader className="px-6 pt-6 pb-5 bg-white border-b border-gray-200">
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                <DialogDescription className="text-sm text-gray-600">{selectedGroup?.domain}</DialogDescription>
                <div className="flex items-center gap-4 mt-3">
                  ...
                  <div className="flex items-center gap-2 text-sm">
                    ...
                    <span className="text-gray-600">
                      {selectedGroup &&
                        new Date(selectedGroup.lastSeen).toLocaleDateString('en-US', {
                          month: 'short',
                          day: 'numeric',
                          year: 'numeric',
                        })}
                    ...
                    >
                      <div className="flex items-start gap-3 mb-3">
                        <Checkbox
                          ...
                          className="mt-1"
                        />
                        <h3 className="font-semibold text-gray-900 flex-1 line-clamp-2">{subject}</h3>
                      ...
                      {snippet && <p className="text-sm text-gray-600 line-clamp-2 leading-relaxed">{snippet}</p>}
                    </div>
                  );
                })}
              </div>
          ...
          <div className="px-6 py-4 bg-white border-t flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              {selectedEmailIds.size > 0 && (
                <div className="flex items-center gap-2">
                  <span className="text-sm font-medium text-blue-600">{selectedEmailIds.size} selected</span>
                  <Button
